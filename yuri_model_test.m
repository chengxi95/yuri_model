...
PFCD_VA_factor = 2.6;
PFCD_MD_factor = 4000; % 3500 worked
VA_pPFC_factor = 2;
MD_pPFC_factor = 47.6; %  47.6 good for 10
% MD_pPFC_factor = 0.7; % 0.7 large enough
pPFC_MD_factor = 148; % 148 good for 10
...
% PFCD_VA_component = zeros(total_trial_num, length(T)); 
PFCD_MD_component = zeros(numberofneurons, length(T)); 
VA_pPFC_component = zeros(numberofneurons, length(T)); 
MD_pPFC_component = zeros(numberofneurons, length(T)); 
pPFC_MD_component = zeros(numberofneurons, length(T)); 
other_pPFC_component = zeros(numberofneurons, length(T));
total_MD_component = zeros(numberofneurons, length(T));
...
Matrix_DPFC_to_MD (test_neuron_number, test_neuron_number)=1;

% Matrix_PFC_to_MD(1:25, 26:50)=1;
% Matrix_PFC_to_MD(26:50, 1:25)=1;
Matrix_MD_to_PFC= ones(numberofneurons,numberofneurons);%matrix_M;
...
for trial_num = 1:total_trial_num
    ...
    y_PFC_S_BT(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_PFC_S_RC = zeros(numberofneurons,length(T));
    y_PFC_S_RC(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_PFC_S_GC = zeros(numberofneurons,length(T));
    y_PFC_S_GC(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;

    y_PFC_S_YT = zeros(numberofneurons,length(T));
    y_PFC_S_YT(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;

    y_PFC_M = zeros(numberofneurons,length(T));
    y_PFC_M(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;

    y_aPFC_D_BT = zeros(numberofneurons,length(T));
    y_aPFC_D_BT(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_aPFC_D_RC= zeros(numberofneurons,length(T));
    y_aPFC_D_RC(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_aPFC_D_GC= zeros(numberofneurons,length(T));
    y_aPFC_D_GC(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_aPFC_D_YT= zeros(numberofneurons,length(T));
    y_aPFC_D_YT(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_aPFC_D_6 = zeros(numberofneurons,length(T));
    y_aPFC_D_6(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_MD_common = zeros(numberofneurons,length(T));
    y_MD_common(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages

    y_pPFC_D = zeros(numberofneurons,length(T));
    y_pPFC_D(1:numberofneurons,1)=-55+randomness*rand(numberofneurons,1)*5;%initializing the excitatory PFC neurons Voltages
    ...
    Iext_SPFC_YT = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_SPFC_BT = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_SPFC_RC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_SPFC_GC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_DPFC_YT = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_DPFC_BT = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_DPFC_RC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_DPFC_GC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
  
    
    
    Iext_PFC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells




    Iext_PFC_M = 1.5 - 0.01 *(randomness*rand(numberofneurons,1));
    Iext_PFC_remote_ORI = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
%     Iext_PFC_remote_Shape = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_PFC_remote_Shape = 1.5 - 0.1 *(randomness*rand(numberofneurons,1)+0.5);
    Iext_PFC_D = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_PFC_D_2 = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_PFC_D_3 = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_PFC_2 = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
    Iext_PFC_3 = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)); % similar driving current for PFC and IPL cells small variation across cells
%     Iext_MD = 1.5 - 0.005 *randomness*rand(numberofneurons,1); % same driving current for all thalamic cells
    Iext_MD = 1.5 - 0.1*(randomness*rand(numberofneurons,1));
    Iext_VA = 1.5 - 0.01 *randomness*rand(numberofneurons,1); % same driving current for all thalamic cells
    Iext_Inh = 1.5 -  0.01 *(randomness*rand(numberofneurons,1)-0.5); % driving current for inhibitory cells similar in PFC and IPL
    Iext_Inh_pPFC = 1.5 - 0.01 *(randomness*rand(numberofneurons,1)-0.5); %driving current for inhibitory cells similar in PFC and IPL
    Iext_Inh_PV = 1.49 - 0.0 *(randomness*rand(numberofneurons,1));
    Iext_response = 1.5 - 0.01 *(randomness*rand(numberofneurons,1));
    ...
    Matrix_amplification = 2;
        ...
        for i= 2:length(T)- MD_delay
            ...
            Isyn_aPFC_D5(:,i+ delay) = Isyn_aPFC_D5(:,i+ delay) +  PFCD_VA_factor * W_PFC_TH .* sum(((i-last_spike_PFC_D_BT<spikewidth) & (i-last_spike_PFC_D_BT>0) & (PFC_VA_matrix ~= 0)), 1).';
            
            Isyn_aPFC_D5(:,i+ delay) = Isyn_aPFC_D5(:,i+ delay) +  PFCD_VA_factor * W_PFC_TH .* sum(((i-last_spike_aPFC_D_RC<spikewidth) & (i-last_spike_aPFC_D_RC>0) & (PFC_VA_matrix ~= 0)), 1).';
           ...
           %%%%  ORIENTATION
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PFC deep  to MD 
    % SHAPE 
            Isyn_PFC_D_MD_shape(:,i+ MD_delay) = Isyn_PFC_D_MD_shape(:,i+ MD_delay) + PFCD_MD_factor * W_PFC_MD .* sum(((i-last_spike_PFC_D_BT<spikewidth) & (i-last_spike_PFC_D_BT>0) & (Matrix_DPFC_to_MD ~= 0)), 1).';
            
            Isyn_PFC_D_MD_shape(:,i+ MD_delay) = Isyn_PFC_D_MD_shape(:,i+ MD_delay) + PFCD_MD_factor * W_PFC_MD .* sum(((i-last_spike_aPFC_D_RC<spikewidth) & (i-last_spike_aPFC_D_RC>0) & (Matrix_DPFC_to_MD ~= 0)), 1).';

            PFCD_MD_component(:, i+ MD_delay) = PFCD_MD_factor * W_PFC_MD .* sum(((i-last_spike_PFC_D_BT<spikewidth) & (i-last_spike_PFC_D_BT>0) & (Matrix_DPFC_to_MD ~= 0)), 1).' + PFCD_MD_factor * W_PFC_MD .* sum(((i-last_spike_aPFC_D_RC<spikewidth) & (i-last_spike_aPFC_D_RC>0) & (Matrix_DPFC_to_MD ~= 0)), 1).';
    
            Isyn_PFC_D_MD_shape(:,i+ MD_delay) = Isyn_PFC_D_MD_shape(:,i+ MD_delay) + pPFC_MD_factor * W_PFC_MD .* sum(((i-last_spike_pPFC_remote_shape<spikewidth) & (i-last_spike_pPFC_remote_shape>0) & (Matrix_PFC_to_MD ~= 0)), 1).';

            pPFC_MD_component(:, i+ MD_delay) = pPFC_MD_factor * W_PFC_MD .* sum(((i-last_spike_pPFC_remote_shape<spikewidth) & (i-last_spike_pPFC_remote_shape>0) & (Matrix_PFC_to_MD ~= 0)), 1).';
    ...
    % ORIENTATION 
            ...
            if ~VA_off
                ...
                Isyn_VA_Matrix_to_PFC_exc_shape(:,i+ delay) = Isyn_VA_Matrix_to_PFC_exc_shape(:,i+ delay) +  VA_pPFC_factor * W_VA_exi .* sum(((i-last_spike_VA_matrix_shape<spikewidth_VA) & (i-last_spike_VA_matrix_shape>0) & (matrix_VA_to_rule ~= 0)), 1).';
            ...
            if ~MD_off
                ...
                Isyn_MD_shape_to_PFC(:,i+ delay) = Isyn_MD_shape_to_PFC(:,i+ delay) + MD_pPFC_factor * W_MD_PFC .* sum(((i-last_spike_MD_shape<spikewidth_MD) & (i-last_spike_MD_shape>0) & (Matrix_MD_to_PFC ~= 0)), 1).'; 
...
%                     spiketimes_M=[spiketimes_M;i,j];
                ...
                if (last_spike_PFC_S_BT(j)~=10^10 && (i-last_spike_PFC_S_BT(j))>tref)
                    ...
                    y_PFC_S_BT(j,i) = y_PFC_S_BT(j,i-1)+leaky_coef*((E_L-y_PFC_S_BT(j,i-1))/tha)*dt+(Iext_SPFC_BT(j)+ Isyn_PFC_M_S (j, i)+ (Isyn_aPFC_local_BT(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i) + Isyn_MD_shape_to_random_PFC_BT(j,i))*dt*(RM/tha);%- Isyn_PV(j, i)
...
%                     spiketimes_S=[spiketimes_S;i,j];
                ...
                if (last_spike_PFC_S_RC(j)~=10^10 && (i-last_spike_PFC_S_RC(j))>tref)
                    ...
                    y_PFC_S_RC(j,i) = y_PFC_S_RC(j,i-1)+ leaky_coef*((E_L-y_PFC_S_RC(j,i-1))/tha)*dt + (Iext_SPFC_RC(j)+ (Isyn_aPFC_local_RC(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i) +Isyn_MD_shape_to_random_PFC_RC(j,i))*dt*(RM/tha);%- Isyn_PV(j, i)
...
%                     spiketimes_S_blue= [spiketimes_S_blue;i,j];
                ...
                if (last_spike_PFC_S_GC(j)~=10^10 && (i-last_spike_PFC_S_GC(j))>tref)
                    ...
                    y_PFC_S_GC(j,i) = y_PFC_S_GC(j,i-1)+ leaky_coef*((E_L-y_PFC_S_GC(j,i-1))/tha)*dt + (Iext_SPFC_GC(j) + (Isyn_aPFC_local_GC(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i) +Isyn_MD_shape_to_random_PFC_GC(j,i))*dt*(RM/tha);%- Isyn_PV(j, i)
...
%                     spiketimes_S_red= [spiketimes_S_red;i,j];
                ...
                if (last_spike_PFC_S_YT(j)~=10^10 && (i-last_spike_PFC_S_YT(j))>tref)
                    ...
                    y_PFC_S_YT(j,i) = y_PFC_S_YT(j,i-1)+ leaky_coef*((E_L-y_PFC_S_YT(j,i-1))/tha)*dt + (Iext_SPFC_YT(j) + (Isyn_aPFC_local_YT(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) - Isyn_PV(j, i)+Isyn_MD_shape_to_random_PFC_YT(j,i))*dt*(RM/tha);%- Isyn_PV(j, i)
...
%                     spiketimes_VA_shape=[spiketimes_VA_shape;i,j];
                ...
                total_MD_component(j, i) = Iext_MD(j)+ I_noise_md_2(j,i)+ Isyn_PFC_D_MD_shape(j,i);
...
%                     spiketimes_ipl_inh=[spiketimes_ipl_inh;i,j];
                ...
                VA_pPFC_component(j, i) = Isyn_VA_Matrix_to_PFC_exc_shape(j, i);
                MD_pPFC_component(j , i) = Isyn_MD_shape_to_PFC(j,i);
                other_pPFC_component(j, i) = Iext_PFC_remote_Shape(j)+  Isyn_aPFC_local_shared (j,i) * Isyn_VA_Matrix_to_PFC(j, i) + I_noise_PFC_6(j,i) + Isyn_aPFC_local_shape(j,i)* Isyn_VA_Matrix_to_PFC(j,i)  - Isyn_PV(j, i);
...
%                     spiketimes_FS_inh=[spiketimes_FS_inh;i,j];
        ...
        % save the results for one trial
        full_PFC_S_BT(trial_num, :) = y_PFC_S_BT(1, :) > v_th; 
        full_PFC_S_RC(trial_num, :) = y_PFC_S_RC(1, :) > v_th; 
        full_PFC_S_GC(trial_num, :) = y_PFC_S_GC(1, :) > v_th; 
        full_PFC_S_YT(trial_num, :) = y_PFC_S_YT(1, :) > v_th; 
        
        full_PFC_D_BT(trial_num, :) = y_aPFC_D_BT(1, :) > v_th; 
        full_PFC_D_RC(trial_num, :) = y_aPFC_D_RC(1, :) > v_th; 
        full_PFC_D_GC(trial_num, :) = y_aPFC_D_GC(1, :) > v_th; 
        full_PFC_D_YT(trial_num, :) = y_aPFC_D_YT(1, :) > v_th; 
        
        full_VA_shape(trial_num, :) = y_VA_matrix_shape(1, :) > v_th;
        full_VA_ori(trial_num, :) = y_VA_matrix_Orientation(1, :) > v_th; 
        
        full_MD_shape(trial_num, :) = y_MD_core_shape(1, :) > v_th;
        full_MD_ori(trial_num, :) = y_MD_core_ori(1, :) > v_th; 
...
subplot(4,1,1)
spy( full_PFC_S_BT, 8,'b'),title('PFC Superficial blue triangle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')


subplot(4,1,2)
spy( full_PFC_S_RC, 8,'b'),title('PFC Superficial red circle ', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(4,1,3)
spy( full_PFC_S_GC, 8,'b'),title('PFC Superficial green circle ', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(4,1,4)
spy( full_PFC_S_YT,8,'b'),title('PFC Superficial yellow triangle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

%
figure (2)
subplot(4,1,1)
spy( full_PFC_D_BT,8,'k'),title('PFC Deep blue triangle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(4,1,2)
spy( full_PFC_D_RC,8,'k'),title('PFC Deep red circle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(4,1,3)
spy( full_PFC_D_GC,8,'k'),title('PFC Deep green circle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(4,1,4)
spy( full_PFC_D_YT,8,'k'),title('PFC Deep yellow triangle', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')


figure( 3)
subplot(6,1,1)
spy( full_VA_shape,8,'k'),title('VA Thalamus Shape', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(6,1,2)
spy( full_VA_ori,8,'k'),title('VA Thalamus Orientation', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(6,1,3)
spy( full_PFC_remote_shape,8,'b'),title('remote PFC Shape', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(6,1,4)
spy( full_PFC_remote_ori,8,'b'),title('remote PFC Ori', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(6,1,5)
spy( full_MD_shape,8,'k'),title('MD Shape', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')

subplot(6,1,6)
spy( full_MD_ori,8,'k'),title('MD ori', 'FontSize', 16)
set(gca,'DataAspectRatio',[10000 1 1]),ylabel('Trial number', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
...
pPFC_firing = y_pPFC_Shape>-50;
MD_firing = y_MD_core_shape>-50;
VA_firing = y_VA_matrix_shape>-50;
aPFC_D_BT_firing = y_aPFC_D_BT>-50;
aPFC_D_RC_firing = y_aPFC_D_RC>-50;

figure (6)
subplot(6,1,1)
plot(PFCD_MD_component(test_neuron_number, :), 'k'), title('PFCD MD', 'FontSize', 16)

subplot(6,1,2)
plot(pPFC_MD_component(test_neuron_number, :), 'k'), title('pPFC MD', 'FontSize', 16)

subplot(6,1,3)
plot(total_MD_component(test_neuron_number, :), 'k'), title('total MD', 'FontSize', 16)
hold on 
subplot(6,1,3)
plot(total_MD_component(2, :), 'g')
hold on 
subplot(6,1,3)
plot(total_MD_component(3, :), 'r')

subplot(6, 1, 4)
plot(VA_pPFC_component(test_neuron_number, :), 'k'), title('VA pPFC', 'FontSize', 16)

subplot(6, 1, 5)
plot(MD_pPFC_component(test_neuron_number, :), 'k'), title('MD pPFC', 'FontSize', 16)

subplot(6, 1, 6)
plot(VA_pPFC_component(test_neuron_number, :) + MD_pPFC_component(1, :) + other_pPFC_component(1, :), 'k'), title('total pPFC', 'FontSize', 16)
hold on
plot(VA_pPFC_component(2, :) + MD_pPFC_component(2, :) + other_pPFC_component(2, :), 'g'), title('total pPFC', 'FontSize', 16)
hold on
plot(VA_pPFC_component(3, :) + MD_pPFC_component(3, :) + other_pPFC_component(3, :), 'r'), title('total pPFC', 'FontSize', 16)
...
filtered_VA_shape_num = zeros(total_trial_num, length(T));
filtered_MD_shape_num = zeros(total_trial_num, length(T));
filtered_PFC_remote_shape_num = zeros(total_trial_num, length(T));

for trial_num = 1:total_trial_num
    filtered_VA_shape_num(trial_num, :) =  conv_gaussian(full_VA_shape_num(trial_num, :), srate,gauss_width);
    filtered_MD_shape_num(trial_num, :) = conv_gaussian(full_MD_shape_num(trial_num, :), srate,gauss_width);
    filtered_PFC_remote_shape_num(trial_num, :) = conv_gaussian(full_PFC_remote_shape_num(trial_num, :), srate,gauss_width);
end

surf(filtered_PFC_remote_shape_num), xlim([st, final])
%%
figure(1)
