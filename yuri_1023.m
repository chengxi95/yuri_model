...
% SHAPE 
        ...
        Isyn_PFC_D_MD_shape(:,i+ 10*MD_delay) = Isyn_PFC_D_MD_shape(:,i+ 10*MD_delay) + 3 * W_PFC_MD .* sum(((i-last_spike_pPFC_remote_shape<spikewidth) & (i-last_spike_pPFC_remote_shape>0) & (Matrix_PFC_to_MD ~= 0)), 1).';
...
%                 spiketimes_FS_inh=[spiketimes_FS_inh;i,j];
    ...
    S_BT = y_PFC_S_BT(1, :) > v_th;
    S_RC = y_PFC_S_RC(1, :) > v_th;
    S_GC = y_PFC_S_GC(1, :) > v_th;
    S_YT = y_PFC_S_YT(1, :) > v_th;
    D_BT = y_aPFC_D_BT(1, :) > v_th;
    D_RC = y_aPFC_D_RC(1, :) > v_th;
    D_GC = y_aPFC_D_GC(1, :) > v_th;
    D_YT = y_aPFC_D_YT(1, :) > v_th;
    VA_shape = y_VA_matrix_shape(1, :) > v_th;
    VA_ori = y_VA_matrix_shape(1, :) > v_th;
    MD_shape = y_MD_core_shape(1, :) > v_th;
    MD_ori = y_MD_core_ori(1, :) > v_th;
    pPFC_shape = y_pPFC_Shape(1, :) > v_th;
    pPFC_ori = y_pPFC_Orientation(1, :) > v_th;
    ...
    VA_shape_firing =  get_firing_num(y_VA_matrix_shape(VA_neuron_index, :) > v_th, gauss_width);
    MD_shape_firing = get_firing_num(y_MD_core_shape(MD_neuron_index, :) > v_th, gauss_width);
    pPFC_shape_firing = get_firing_num(y_pPFC_Shape(pPFC_neuron_index, :) > v_th, gauss_width);
    
    save(sprintf('sima_1023_trial_%d', rr), ...
        'S_BT', ...
        'S_RC', ...
        'S_GC', ...
        'S_YT', ...
...
for i = 1:total_trial_num
    S = load(sprintf('sima_1023_trial_%d', i));
    full_PFC_S_BT(i, :) = S.('S_BT');
    full_PFC_S_RC(i, :) = S.('S_RC');
    full_PFC_S_GC(i, :) = S.('S_GC');
    full_PFC_S_YT(i, :) = S.('S_YT');
    full_PFC_D_BT(i, :) = S.('D_BT');
    full_PFC_D_RC(i, :) = S.('D_RC');
    full_PFC_D_GC(i, :) = S.('D_GC');
    full_PFC_D_YT(i, :) = S.('D_YT');
    full_VA_shape(i, :) = S.('VA_shape');
    full_VA_ori(i, :) = S.('VA_ori');
    full_MD_shape(i, :) = S.('MD_shape');
    full_MD_ori(i, :) = S.('MD_ori');
    full_PFC_remote_shape(i, :) = S.('pPFC_shape');
    full_PFC_remote_ori(i, :) = S.('pPFC_ori');
    full_VA_shape_num(i, :) =  S.('VA_shape_firing');
    full_MD_shape_num(i, :) = S.('MD_shape_firing');
    full_PFC_remote_shape_num(i, :) = S.('pPFC_shape_firing');
end
