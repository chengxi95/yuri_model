% use fixed initialization to generate certain number of trials data, save
% LFP information
close all;
...
T = 0:dt:t_final;
end_time = 4500/dt; % there is a delay between spike and corresponding current, only saves the results till end_time
...
% placeholder for multi trials LFP
lfp_PFC_M = zeros(total_trial_num, length(T));
lfp_PFC_S = zeros(total_trial_num, length(T));
lfp_PFC_D = zeros(total_trial_num, length(T));
lfp_VA = zeros(total_trial_num, length(T));
lfp_MD = zeros(total_trial_num, length(T));
    ...
    for i= 2:length(T)- MD_delay*10
       ...
       %%%%%% Midle layer (l = 1) to superficial layer (l = 2) %%%%%%%%%%%%
        ...
        for j = 1:numberofneurons
            ...
            lfp_PFC_M(rr, i) = lfp_PFC_M(rr, i) + I_stim(j,i)+ I_noise_PFC_1(j, i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) +  Isyn_PFC_M_BT (j, i)+ (Isyn_aPFC_local_BT(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i) + Isyn_MD_shape_to_random_PFC_BT(j,i)+Isyn_MD_orientation_to_random_PFC_BT(j,i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) +  Isyn_PFC_M_BT (j, i)+ (Isyn_aPFC_local_BT(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) + Isyn_MD_shape_to_random_PFC_BT(j,i)+Isyn_MD_orientation_to_random_PFC_BT(j,i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_RC (j, i)+ (Isyn_aPFC_local_RC(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i) +Isyn_MD_shape_to_random_PFC_RC(j,i)+Isyn_MD_orientation_to_random_PFC_RC(j,i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_RC (j, i)+ (Isyn_aPFC_local_RC(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) +Isyn_MD_shape_to_random_PFC_RC(j,i)+Isyn_MD_orientation_to_random_PFC_RC(j,i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_GC (j, i) + (Isyn_aPFC_local_GC(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)- Isyn_PV(j, i)+Isyn_MD_shape_to_random_PFC_GC(j,i)+Isyn_MD_orientation_to_random_PFC_GC(j,i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_GC (j, i) + (Isyn_aPFC_local_GC(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)+Isyn_MD_shape_to_random_PFC_GC(j,i)+Isyn_MD_orientation_to_random_PFC_GC(j,i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_YT (j, i) + (Isyn_aPFC_local_YT(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) - Isyn_PV(j, i)+Isyn_MD_shape_to_random_PFC_YT(j,i)+Isyn_MD_orientation_to_random_PFC_YT(j,i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_PFC_M_YT (j, i) + (Isyn_aPFC_local_YT(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i)+Isyn_MD_shape_to_random_PFC_YT(j,i)+Isyn_MD_orientation_to_random_PFC_YT(j,i);
            ...
            lfp_PFC_D(rr, i) = lfp_PFC_D(rr, i) + (W11 * Isyn_aPFC_to_D_BT(j,i)+ W12 * Isyn_aPFC_to_D_RC(j,i)+ W13 *Isyn_aPFC_to_D_GC(j,i)+ W14* Isyn_aPFC_to_D_YT(j,i)) + Isyn_aPFC_D_shared(j,i) +I_noise_PFC_2(j, i);
            ...
            lfp_PFC_D(rr, i) = lfp_PFC_D(rr, i) + (W21 * Isyn_aPFC_to_D_BT(j,i)+ W22 * Isyn_aPFC_to_D_RC(j,i)+ W23 *Isyn_aPFC_to_D_GC(j,i)+ W24* Isyn_aPFC_to_D_YT(j,i)) +Isyn_aPFC_D_shared(j,i) +I_noise_PFC_2(j, i);
            ...
            lfp_PFC_D(rr, i) = lfp_PFC_D(rr, i) + (W31 * Isyn_aPFC_to_D_BT(j,i)+ W32 * Isyn_aPFC_to_D_RC(j,i)+ W33 *Isyn_aPFC_to_D_GC(j,i)+ W34* Isyn_aPFC_to_D_YT(j,i)) + Isyn_aPFC_D_shared(j,i) +I_noise_PFC_2(j, i);
            ...
            lfp_PFC_D(rr, i) = lfp_PFC_D(rr, i) + (W41 * Isyn_aPFC_to_D_BT(j,i)+ W42 * Isyn_aPFC_to_D_RC(j,i)+ W43 *Isyn_aPFC_to_D_GC(j,i)+ W44* Isyn_aPFC_to_D_YT(j,i))+ Isyn_aPFC_D_shared(j,i) +I_noise_PFC_2(j, i);
            ...
            if ~VA_off
                ...
                lfp_VA(rr, i) = lfp_VA(rr, i) + I_noise_VA(j,i)+ Isyn_aPFC_D5_shape(j, i) - Isyn_SNpr_to_VA(j,i);
                ...
                lfp_VA(rr, i) = lfp_VA(rr, i) + I_noise_VA(j,i)+ Isyn_aPFC_D5_orientation(j, i) - Isyn_SNpr_to_VA(j,i);
            ...
            if ~MD_off
                ...
                lfp_MD(rr, i) = lfp_MD(rr, i) + I_noise_md_2(j,i)+ Isyn_PFC_D_MD_shape(j,i);
                ...
                lfp_MD(rr, i) = lfp_MD(rr, i) + I_noise_md_1(j,i)+ Isyn_PFC_D_MD_ori(j, i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_aPFC_local_shared (j,i) * Isyn_VA_Matrix_to_PFC(j, i) + I_noise_PFC_6(j,i)+ Isyn_VA_Matrix_to_PFC_exc_shape(j, i)  +  Isyn_MD_shape_to_PFC(j,i) + Isyn_aPFC_local_shape(j,i)* Isyn_VA_Matrix_to_PFC(j,i) - Isyn_FS_shape(j, i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + Isyn_aPFC_local_shared (j,i) * Isyn_VA_Matrix_to_PFC(j, i) + I_noise_PFC_6(j,i)+ Isyn_VA_Matrix_to_PFC_exc_shape(j, i)  +  Isyn_MD_shape_to_PFC(j,i) + Isyn_aPFC_local_shape(j,i)* Isyn_VA_Matrix_to_PFC(j,i);
            ...
            % lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + I_noise_PFC_6(j,i)+ Isyn_aPFC_local_shared (j,i) * Isyn_VA_Matrix_to_PFC(j, i) + Isyn_VA_Matrix_to_PFC_exc_orientation(j, i) +Isyn_MD_ori_to_PFC(j,i) + Isyn_aPFC_local_ori(j,i)* Isyn_VA_Matrix_to_PFC(j,i) - Isyn_FS_ori(j, i);
            lfp_PFC_S(rr, i) = lfp_PFC_S(rr, i) + I_noise_PFC_6(j,i)+ Isyn_aPFC_local_shared (j,i) * Isyn_VA_Matrix_to_PFC(j, i) + Isyn_VA_Matrix_to_PFC_exc_orientation(j, i) +Isyn_MD_ori_to_PFC(j,i) + Isyn_aPFC_local_ori(j,i)* Isyn_VA_Matrix_to_PFC(j,i);
...
lfp_PFC_M = lfp_PFC_M(:, 1:end_time);
lfp_PFC_S = lfp_PFC_S(:, 1:end_time);
lfp_PFC_D = lfp_PFC_D(:, 1:end_time);
lfp_VA = lfp_VA(:, 1:end_time);
lfp_MD = lfp_MD(:, 1:end_time);
