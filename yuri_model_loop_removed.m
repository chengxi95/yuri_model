...
%   Time constants
tha = 20; %time constant of cortical cells
tau_md = 20; %100 time constant of thalamic population I projecting to IPL
...
W11=0.7; W12=0.5; W13=0.1; W14= 0.1;
W21=0.5; W22=0.7; W23=0.1; W24= 0.1;
W31=0.1; W32=0.1; W33=0.7; W34= 0.5;
W41=0.1; W42=0.1; W43=0.5; W44= 0.7;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
t_start_stim_1_abs = 1250; % cue time
cue_amp = 1;
abs_stim_duration = 400; %0.1;% stim duration is one ms
...
t_start_stim_1_abs = 2750;% cue time
cue_amp = 0;
abs_stim_duration = 0.1;% stim duration is one ms
stim_duration = abs_stim_duration/dt;
abs_stim_intervals = 0;
stim_intervals = abs_stim_intervals/dt;
t_start_stim_1 = t_start_stim_1_abs/dt;
for i= 1:no_of_trl
    I_stim_con(1:column_length,t_start_stim_1:t_start_stim_1+stim_duration) = cue_amp;% first stimulus delivered to visual starter neuron
    t_start_stim_1 = t_start_stim_1+1000/dt;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% deep to MD
W_PFC_MD =  0.0175;% 0.0175;% 0.05;
W_PFC_to_str= 0.015;%0.03
PFC_MD_matrix = zeros(numberofneurons,numberofneurons);
connectivity_radius = 5;


for  ii = 1:column_length: numberofneurons-column_length
    PFC_MD_matrix(ii:ii+ column_length - 1,ii:ii+ column_length - 1) = W_PFC_MD;
end

PFC_VA_matrix = zeros(numberofneurons,numberofneurons);
    PFC_VA_matrix(1: 20, 1:numberofneurons) = W_PFC_MD;
...
for rr= 1
    ...
    y_VA_matrix = zeros(numberofneurons,length(T));
    y_VA_matrix(1:numberofneurons,1)=-50+rand(numberofneurons,1)*1; %initializing the thalamic neurons Voltages
    ...
    y_MD1_core = zeros(numberofneurons,length(T));
    y_MD1_core(1:numberofneurons,1)=-50+rand(numberofneurons,1)*1; %initializing the thalamic neurons Voltages
    y_MD2_core = zeros(numberofneurons,length(T));
    y_MD2_core(1:numberofneurons,1)=-50+rand(numberofneurons,1)*1; %initializing the thalamic neurons Voltages
    ...
    y_ST_inh(1:numberofneurons,1)=-50+rand(numberofneurons,1)*1;% initializing the PFC neurons Voltages
    ...
    last_spike_aPFC_D = 10^10*ones(numberofneurons,1);
    ...
    last_spike_VA_matrix = 10^10*ones(numberofneurons,1);
    ...
    last_spike_MD = 10^10*ones(numberofneurons,1);
    ...
    Isyn_aPFC_local = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_shared = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_BT= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_RC= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_GC= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_YT= zeros(numberofneurons,length(T)+ delay);

    Isyn_PFC_M_S= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D_shared = zeros(numberofneurons,length(T)+ delay);

    Isyn_pPFC_D = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_S_to_D = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_local = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_local_left = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFCD_to_response = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_IPL = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_Exc_to_Inh = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_Inh_to_exc = zeros(numberofneurons,length(T)+ delay);
    Isyn_SNpr_to_VA = zeros(numberofneurons,length(T)+ delay);
    Isyn_IPL = zeros(numberofneurons,length(T)+ delay);
    Isyn_IPL_PFC = zeros(numberofneurons,length(T)+ delay);
    Isyn_IPL_exc_inh = zeros(numberofneurons,length(T)+ delay);
    Isyn_IPL_loc_remote = zeros(numberofneurons,length(T)+ delay);
    Isyn_IPL_md = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_cmt = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D_MD = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_D_MD = zeros(numberofneurons,length(T)+ delay);
    Isyn_MD_to_inh = zeros(numberofneurons,length(T)+ delay);
    Isyn_MD_to_inh_PV = zeros(numberofneurons,length(T)+ delay);
    Isyn_MD1_to_aPFC= ones(numberofneurons,length(T)+ delay);
    Isyn_MD2_to_pPFC = ones(numberofneurons,length(T)+ delay);
    Isyn_VA_Matrix_to_PFC = ones(numberofneurons,length(T)+ delay);
    Isyn_ST = zeros(numberofneurons,length(T)+ delay);
    Isyn_PV = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_local_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D_effective_2 = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_local_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_local_effective_2 = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_local_effective_3 = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_S_to_D_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFC_D_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_pPFCD_to_response_effective = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D5 = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D5_to_ST = zeros(numberofneurons,length(T)+ delay);
    Isyn_VA_Matrix_to_Inh= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D5_toremote = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_D5_shape = zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D_RC= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D_GC= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D_YT= zeros(numberofneurons,length(T)+ delay);
    Isyn_aPFC_to_D_BT= zeros(numberofneurons,length(T)+ delay);
    ...
    for i= 2:length(T)
        ...
        w_within_chains = 0.6;
        % Layer 2 to layer 2
        % local PFC excitatory to excitatory
        ...
        Isyn_aPFC_D_shared(:,i+ delay) = Isyn_aPFC_D_shared(:,i+ delay) + w_across_chains .* sum(((i-last_spike_aPFC_D<spikewidth) & (i-last_spike_aPFC_D>0)) .* matrix_local, 1).';
        ...
        Isyn_pPFC_local(:,i+ delay) = Isyn_pPFC_local(:,i+ delay) + sum(((i-last_spike_pPFC_remote_ORI<spikewidth) & (i-last_spike_pPFC_remote_ORI>0)) .* matrix_local, 1).';
        ...
        % no error 
        Isyn_pPFC_local_left(:,i+ delay) = Isyn_pPFC_local_left(:,i+ delay) + sum(((i-last_spike_pPFC_remote_ORI_LEFT<spikewidth) & (i-last_spike_pPFC_remote_ORI_LEFT>0)) .* matrix_local, 1).';
        ...
        % 8 not useful
        % Isyn_MD1_to_aPFC(:,i+ delay) = amplification.*(sum((i-last_spike_MD<spikewidth_MD) & (i-last_spike_MD>0) & (matrix_MD_to_Crtex~=0), 1)~=0).';

        % Isyn_MD2_to_pPFC(:,i+ delay) = amplification.*(sum((i-last_spike_MD2_core<spikewidth_MD) & (i-last_spike_MD2_core>0) & (matrix_MD_to_Crtex~=0), 1)~=0).';
        
        % 8
        Isyn_VA_Matrix_to_PFC(sum(((i-last_spike_VA_matrix<spikewidth_MD) & (i-last_spike_VA_matrix>0)) .* matrix_MD_to_Crtex, 1)~=0,i+ delay) = Matrix_amplification;



        % PFC exc to IPL excitatory (long distance connections)
        
        Isyn_aPFC_D_MD(:,i+ delay) = Isyn_aPFC_D_MD(:,i+ delay) + sum(((i-last_spike_aPFC_D6<spikewidth) & (i-last_spike_aPFC_D6>0)) .* PFC_MD_matrix, 1).';

        Isyn_aPFC_D5(:,i+ delay) = Isyn_aPFC_D5(:,i+ delay) +  W_PFC_MD .* sum(((i-last_spike_aPFC_D<spikewidth) & (i-last_spike_aPFC_D>0) & (PFC_VA_matrix ~= 0)), 1).';
        
        Isyn_aPFC_D5(:,i+ delay) = Isyn_aPFC_D5(:,i+ delay) +  W_PFC_MD .* sum(((i-last_spike_aPFC_D_RC<spikewidth) & (i-last_spike_aPFC_D_RC>0) & (PFC_VA_matrix ~= 0)), 1).';

        Isyn_aPFC_D5_shape(:,i+ delay) = Isyn_aPFC_D5_shape(:,i+ delay) +  W_PFC_MD .* sum((i-last_spike_aPFC_D_GC<spikewidth) & (i-last_spike_aPFC_D_GC>0), 1).';

        Isyn_aPFC_D5_shape(:,i+ delay) = Isyn_aPFC_D5_shape(:,i+ delay) +  W_PFC_MD .* sum((i-last_spike_aPFC_D_YT<spikewidth) & (i-last_spike_aPFC_D_YT>0), 1).';

        Isyn_aPFC_D5_toremote(:,i+ delay) = Isyn_aPFC_D5_toremote(:,i+ delay) +  0.3 .* W_PFC_MD.*sum((i-last_spike_aPFC_D<spikewidth) & (i-last_spike_aPFC_D>0), 1).';

        Isyn_aPFC_D5_to_ST(:,i+ delay) = Isyn_aPFC_D5_to_ST(:,i+ delay) +  W_PFC_to_str .* sum((i-last_spike_aPFC_D<spikewidth) & (i-last_spike_aPFC_D>0), 1).';
        ...
        Isyn_VA_Matrix_to_Inh(:,i+ delay) = Isyn_VA_Matrix_to_Inh(:,i+ delay) +  W_PFC_to_str .* sum((i-last_spike_VA_matrix<spikewidth) & (i-last_spike_VA_matrix>0), 1).';

        Isyn_pPFC_D_MD(:,i+ delay) = Isyn_pPFC_D_MD(:,i+ delay) + sum(((i-last_spike_pPFC_D<spikewidth) & (i-last_spike_pPFC_D>0)) .* PFC_MD_matrix, 1).';
        ...
        for j = 1:numberofneurons
            ...
            if (last_spike_PFC_S_BT(j)~=10^10 && (i-last_spike_PFC_S_BT(j))>tref)
                ...
                y_PFC_S_BT(j,i) = y_PFC_S_BT(j,i-1)+leaky_coef*((E_L-y_PFC_S_BT(j,i-1))/tha)*dt+(Iext_PFC(j)+ Isyn_PFC_M_S (j, i)+ (Isyn_aPFC_local_GC(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) )*dt*(RM/tha);%- Isyn_PV(j, i)
            ...
            % Isyn_aPFC_local_shared _effective(j,i) = Isyn_aPFC_local_shared (j,i)*Isyn_MD1_to_aPFC(j, i);
            % Isyn_aPFC_D_effective(j,i) = Isyn_aPFC_D(j,i)*Isyn_MD1_to_aPFC(j, i);

            % membrane potential across PFC cells

            if (last_spike_PFC_S_RC(j)~=10^10 && (i-last_spike_PFC_S_RC(j))>tref)

                y_PFC_S_RC(j,i) = y_PFC_S_RC(j,i-1)+ leaky_coef*((E_L-y_PFC_S_RC(j,i-1))/tha)*dt + (Iext_PFC(j)+ (Isyn_aPFC_local_RC(j, i) + Isyn_aPFC_local_shared (j,i))* Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) )*dt*(RM/tha);%- Isyn_PV(j, i)
            ...
            if (last_spike_PFC_S_GC(j)~=10^10 && (i-last_spike_PFC_S_GC(j))>tref)
                ...
                y_PFC_S_GC(j,i) = y_PFC_S_GC(j,i-1)+ leaky_coef*((E_L-y_PFC_S_GC(j,i-1))/tha)*dt + (Iext_PFC(j) + (Isyn_aPFC_local_GC(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) )*dt*(RM/tha);%- Isyn_PV(j, i)
            ...
            if (last_spike_PFC_S_YT(j)~=10^10 && (i-last_spike_PFC_S_YT(j))>tref)
                ...
                y_PFC_S_YT(j,i) = y_PFC_S_YT(j,i-1)+ leaky_coef*((E_L-y_PFC_S_YT(j,i-1))/tha)*dt + (Iext_PFC(j) + (Isyn_aPFC_local_YT(j, i) + Isyn_aPFC_local_shared (j,i)) * Isyn_VA_Matrix_to_PFC(j, i)+ I_noise_PFC_1(j, i) )*dt*(RM/tha);%- Isyn_PV(j, i)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % 3 PFC Deep layer 5
            Isyn_aPFC_to_D_effective(j, i) = Isyn_aPFC_to_D(j,i)*Isyn_MD1_to_aPFC(j, i);
            Isyn_aPFC_D_effective_2(j, i) = Isyn_aPFC_D(j,i)*Isyn_MD1_to_aPFC(j, i);
            ...
            if (last_spike_aPFC_D(j)~=10^10 && (i-last_spike_aPFC_D(j))>tref)

                y_aPFC_D_BT(j,i) = y_aPFC_D_BT(j,i-1)+leaky_coef*((E_L-y_aPFC_D_BT(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ (W11 * Isyn_aPFC_to_D_BT(j,i)+ W12 * Isyn_aPFC_to_D_RC(j,i)+ W13 *Isyn_aPFC_to_D_GC(j,i)+ W14* Isyn_aPFC_to_D_YT(j,i)) *Isyn_MD1_to_aPFC(j, i)+Isyn_aPFC_D_shared(j,i)*Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)

            elseif last_spike_aPFC_D(j)==10^10
            ...
            if (last_spike_aPFC_D_RC(j)~=10^10 && (i-last_spike_aPFC_D_RC(j))>tref)
                ...
                y_aPFC_D_RC(j,i) = y_aPFC_D_RC(j,i-1)+leaky_coef*((E_L-y_aPFC_D_RC(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ (W21 * Isyn_aPFC_to_D_BT(j,i)+ W22 * Isyn_aPFC_to_D_RC(j,i)+ W23 *Isyn_aPFC_to_D_GC(j,i)+ W24* Isyn_aPFC_to_D_YT(j,i))*Isyn_MD1_to_aPFC(j, i)+Isyn_aPFC_D_shared(j,i)*Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)
            ...
            if (last_spike_aPFC_D_GC(j)~=10^10 && (i-last_spike_aPFC_D_GC(j))>tref)
                ...
                y_aPFC_D_GC(j,i) = y_aPFC_D_GC(j,i-1)+leaky_coef*((E_L-y_aPFC_D_GC(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ (W31 * Isyn_aPFC_to_D_BT(j,i)+ W32 * Isyn_aPFC_to_D_RC(j,i)+ W33 *Isyn_aPFC_to_D_GC(j,i)+ W34* Isyn_aPFC_to_D_YT(j,i))*Isyn_MD1_to_aPFC(j, i)+Isyn_aPFC_D_shared(j,i)*Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)
            ...
            if (last_spike_aPFC_D_YT(j)~=10^10 && (i-last_spike_aPFC_D_YT(j))>tref)
                ...
                y_aPFC_D_YT(j,i) = y_aPFC_D_YT(j,i-1)+leaky_coef*((E_L-y_aPFC_D_YT(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ (W41 * Isyn_aPFC_to_D_BT(j,i)+ W42 * Isyn_aPFC_to_D_RC(j,i)+ W43 *Isyn_aPFC_to_D_GC(j,i)+ W44* Isyn_aPFC_to_D_YT(j,i))*Isyn_MD1_to_aPFC(j, i)+ Isyn_aPFC_D_shared(j,i)* Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)
            ...
            % if (last_spike_aPFC_D(j)~=10^10 && (i-last_spike_aPFC_D(j))>tref)
            %
            %     y_aPFC_D(j,i) = y_aPFC_D(j,i-1)+leaky_coef*((E_L-y_aPFC_D(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ Isyn_aPFC_to_D(j,i)*Isyn_MD1_to_aPFC(j, i)+Isyn_aPFC_D(j,i)*Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)
            %
            % elseif last_spike_aPFC_D(j)==10^10
            %
            %     y_aPFC_D(j,i) = y_aPFC_D(j,i-1)+leaky_coef*((E_L-y_aPFC_D(j,i-1))/tha)*dt+(Iext_PFC_D(j)+ Isyn_aPFC_to_D(j,i)*Isyn_MD1_to_aPFC(j, i)+Isyn_aPFC_D(j,i)*Isyn_MD1_to_aPFC(j, i)+I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)
            ...
            % end
            %
            % if y_aPFC_D(j,i)>= v_th
            %     last_spike_aPFC_D(j)= i;
            %     y_aPFC_D(j,i)= 0;
            %     spiketimes_D_BT= [spiketimes_D;i,j];
            %
            % end
            %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if (last_spike_aPFC_D6(j)~=10^10 && (i-last_spike_aPFC_D6(j))>tref)

                y_aPFC_D_6(j,i) = y_aPFC_D_6(j,i-1)+leaky_coef*((E_L-y_aPFC_D_6(j,i-1))/tha)*dt+(Iext_PFC_D(j) + Isyn_aPFC_D(j,i)*Isyn_MD1_to_aPFC(j, i) + I_noise_PFC_2(j, i))*dt*(RM/tha); %+Isyn_IPL_PFC(j,i)-Isyn_aPFC_Inh_to_exc(j,i)

            elseif last_spike_aPFC_D6(j)==10^10
            ...
            if y_aPFC_D_6(j,i)>= v_th
                last_spike_aPFC_D6(j)= i;
                y_aPFC_D_6(j,i)= 0;
                spiketimes_D6 = [spiketimes_D6;i,j];
            ...
            if (last_spike_MD(j)~=10^10 && (i-last_spike_MD(j))>tref)

                y_MD1_core(j,i) = y_MD1_core(j,i-1)+((E_L-y_MD1_core(j,i-1))/tau_md)*dt+(Iext_md(j)+I_noise_md_1(j,i)+ Isyn_aPFC_D_MD(j, i) )*dt*(RM/tau_md);%

            elseif last_spike_MD(j)==10^10
            ...
            % noise_prob_md = noise_prob_PFC;
            % if n1<noise_prob_md
            %     I_noise_md_2(j,i) = noise_amp;
            % else
            %     I_noise_md_2(j,i) = 0;
            % end
            %
            %
            % if (last_spike_MD2_core(j)~=10^10 && (i-last_spike_MD2_core(j))>tref)
            %
            %     y_MD2_core(j,i) = y_MD2_core(j,i-1)+((E_L-y_MD2_core(j,i-1))/tau_md)*dt+(Iext_md(j)+ I_noise_md_2(j,i)+ Isyn_pPFC_D_MD(j,i) )*dt*(RM/tau_md);%
            %
            % elseif last_spike_MD2_core(j)==10^10
            %
            %     y_MD2_core(j,i) = y_MD2_core(j,i-1)+((E_L-y_MD2_core(j,i-1))/tau_md)*dt+(Iext_md(j)+ I_noise_md_2(j,i)+ Isyn_pPFC_D_MD(j,i) )*dt*(RM/tau_md);%
            %
            % else
            ...
            % end
            %
            % if y_MD2_core(j,i) >= v_th
            %     last_spike_MD2_core(j)=i;
            %     y_MD2_core(j,i) = 0;
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % MD matrix cells connecting aPFC to pPFC
            n1 = rand;
            noise_prob_md = noise_prob_PFC;
            if n1<noise_prob_md
            ...
            %matrix MD  cells
            if (last_spike_VA_matrix(j)~=10^10 && (i-last_spike_VA_matrix(j))>tref)

                y_VA_matrix(j,i) = y_VA_matrix(j,i-1)+((E_L-y_VA_matrix(j,i-1))/tau_md)*dt+(Iext_VA(j)+ I_noise_VA(j,i)+ Isyn_aPFC_D5(j, i) - Isyn_SNpr_to_VA(j,i))*dt*(RM/tau_md);%

            elseif last_spike_VA_matrix(j)==10^10
            ...
            if y_VA_matrix(j,i)>=v_th
                last_spike_VA_matrix(j)=i;
                y_VA_matrix(j,i)=0;
            ...
            if (last_spike_VA_matrix_shape(j)~=10^10 && (i-last_spike_VA_matrix_shape(j))>tref)
                ...
                y_VA_matrix_Orientation (j,i) = y_VA_matrix_Orientation (j,i-1)+((E_L-y_VA_matrix_Orientation (j,i-1))/tau_md)*dt+(Iext_VA(j)+ I_noise_VA(j,i)+ Isyn_aPFC_D5_shape(j, i) - Isyn_SNpr_to_VA(j,i))*dt*(RM/tau_md);%
            ...
            %
            % %matrix MD  cells
            % if (last_spike_VA_matrix_Ori(j)~=10^10 && (i-last_spike_VA_matrix_Ori(j))>tref)
            %
            %     y_VA_matrix_Ori(j,i) = y_VA_matrix_Ori(j,i-1)+((E_L-y_VA_matrix_Ori(j,i-1))/tau_md)*dt+(Iext_VA(j)+ I_noise_VA(j,i)+ Isyn_aPFC_D5_Ori(j, i) - Isyn_SNpr_to_VA(j,i))*dt*(RM/tau_md);%
            %
            % elseif last_spike_VA_matrix_Ori(j)==10^10
            %
            %     y_VA_matrix_Ori(j,i) = y_VA_matrix_Ori(j,i-1)+((E_L-y_VA_matrix_Ori(j,i-1))/tau_md)*dt+(Iext_VA(j)+ I_noise_VA(j,i)+ Isyn_aPFC_D5_Ori(j, i) - Isyn_SNpr_to_VA(j,i))*dt*(RM/tau_md);%
            %
            % else
            %
            %     y_VA_matrix_Ori(j,i)=E_L;
            %
            % end
            %
            % if y_VA_matrix_Ori(j,i)>=v_th
            %     last_spike_VA_matrix_Ori(j)=i;
            %     y_VA_matrix_Ori(j,i)=0;
            %     spiketimes_VA_ori=[spiketimes_VA_ori;i,j];
            %
            % end
            ...
            if (last_spike_pPFC_remote_shape(j)~=10^10 && (i-last_spike_pPFC_remote_shape(j))>tref)
                ...
                y_pPFC_Shape(j,i) = y_pPFC_Shape(j,i-1)+leaky_coef*((E_L-y_pPFC_Shape(j,i-1))/tha)*dt+(Iext_PFC_remote_Shape(j)+ I_noise_PFC_6(j,i) + Isyn_aPFC_D5_toremote(j,i)*Isyn_VA_Matrix_to_PFC(j, i) + Isyn_pPFC_local(j,i)*Isyn_MD2_to_pPFC(j, i)  )*dt*(RM/tha); %

            elseif last_spike_pPFC_remote_shape(j)==10^10

                y_pPFC_Shape(j,i) = y_pPFC_Shape(j,i-1)+leaky_coef*((E_L-y_pPFC_Shape(j,i-1))/tha)*dt+(Iext_PFC_remote_Shape(j)+ I_noise_PFC_6(j,i) + Isyn_aPFC_D5_toremote(j,i)*Isyn_VA_Matrix_to_PFC(j, i) + Isyn_pPFC_local(j,i)*Isyn_MD2_to_pPFC(j, i)  )*dt*(RM/tha); %+ Isyn_aPFC_cmt(j,i)- Isyn_pPFC_Inh_to_exc(j,i);
            ...
            if (last_spike_pPFC_remote_Orientation(j)~=10^10 && (i-last_spike_pPFC_remote_Orientation(j))>tref)
                ...
                y_pPFC_Orientation(j,i) = y_pPFC_Orientation(j,i-1)+leaky_coef*((E_L-y_pPFC_Orientation(j,i-1))/tha)*dt+(Iext_PFC_remote_Shape(j)+ I_noise_PFC_6(j,i) + Isyn_aPFC_D5_toremote(j,i)*Isyn_VA_Matrix_to_PFC(j, i)  - Isyn_PV(j, i) )*dt*(RM/tha); %+ Isyn_pPFC_local(j,i)*Isyn_MD2_to_pPFC(j, i)
    ...
    % LFP_MD_matrix(rr,:)= smooth(mean(I_noise_md_1)+ mean(I_noise_md_2)+ mean(I_noise_VA)+ mean(Isyn_aPFC_D_MD)+mean(Isyn_pPFC_D_MD)+ mean(Isyn_aPFC_D_MD));% post-synaptic current in pfc
...
set(gca,'DataAspectRatio',[1000 1 1]),ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(5,1,3)
spy( y_VA_matrix>-50,8,'k'),title('VA Thalamus Shape', 'FontSize', 16)
set(gca,'DataAspectRatio',[1000 1 1]),ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(5,1,4)
...
subplot(8,1,1)
plot( y_PFC_M(1, :),'b'),title('Middle layer sensory', 'FontSize', 16)
ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')


subplot(8,1,2)
plot( y_PFC_S(1, :),'b'),title('PFC Superficial', 'FontSize', 16)
ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
...
set(gca,'DataAspectRatio',[1000 1 1]),ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
subplot(8,1,7)
plot( y_VA_matrix(1, :),'k'),title('VA Thalamus', 'FontSize', 16)
set(gca,'DataAspectRatio',[1000 1 1]),ylabel('Neuron ID', 'FontSize',16), xlim([0 t_final/dt]);%,xlabel('time')
    ...
    for jj=1:max(trl)+1
        ...
        if isempty(temp) == 0
            ...
            for k = 1:nspikes
                ...
                line([x(k) x(k)], [jj-0.5 jj+0.5],'Color','r');  xlim([1 100000]),ylim([0.5  no_of_trials+0.5 ]),ylabel(' # of trials','FontSize',13),title(' deep layer yellow Triangle cells ','FontSize',13);
